...
- [x] **Stores credentials in MongoDB** - Done. Got a lot of the code and references from the simon-db code. Implemented getUser, getUserByToken, addUser, and updateUser functions in database.js. I added calls to these functions to the backend and altered the findUser and createUser functions in the backend. The code now stores the user's email, hashed password, and token in the database. I did not need to associate user credentials and submitted recipes.
...
- [x] **Application is fully functional** - All the desired functions have been implemented and it has been released to AWS. No placeholder code acts in the application (though some remnants that missed cleanup may be present, they are not part of the functionality of the program).
